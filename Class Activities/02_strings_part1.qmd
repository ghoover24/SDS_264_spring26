...
```{r}
str_view(spot_smaller$title, "M") # tells you the position and highlights what you are looking for

str_subset(spot_smaller$title, "M") # creates a new vector including the values you are looking for
```
...
str_detect(spot_smaller$title, "M") # TRUE or FALSE

str_detect("hello", "ll")


spot_smaller |>                               # includes all the other rows to make an actual table
...
```{r}
str_extract("find first vowel", "[aeiou]") # or statement "find me the first a or e or i ..."

str_extract("any numb3rs?", "\\d") # digits, are there any digits in the string?
...
spot_smaller |>
  ...
  mutate(numbers = str_extract(album_name, "\\d")) # will only extract the first number
```
...
### Examples of Regular Expressions
  ...
  c. ^_  -> starts with
  d. _$  -> ends with
  e. \\  -> "escaping" a different function or use (example "\\$" to escape the default use of "\$")
  f. [^ ]-> everything except what is in the square brackets; negation
...
```{r}
str_view(spot_smaller$title, "M")
str_view(spot_smaller$title, "^M") # starts with

str_view(spot_smaller$title, "o")
str_view(spot_smaller$title, "o$") # ends with
```
