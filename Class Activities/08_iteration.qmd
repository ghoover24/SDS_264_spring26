...
1. **output** - plan ahead and allocate enough space for output, what do you want the for loop to give you?, create an empty space (matrix, vector, df, etc.)
2. **sequence** - determines what to loop over; cycles through different values of $i$, how long should it run?, 
3. **body** - code that does the work; run repeatedly with different values of $i$, does the work
...
df[1]           # this is a list (/a tibble) and you can't take the median of a list
df[[1]]
...
output <- vector("double", ncol(df))  # 1. output; ncol = the number of columns
for (i in 1:4) {                      # 2. sequence
  output[[i]] <- median(df[[i]])      # 3. body; save the output
}
output

# ?seq_along - a safer option if had zero length vectors
output <- vector("double", ncol(df))  # 1. output
for (i in seq_along(df)) {            # 2. sequence; seq_along = start at the first, end at the last
...
output <- vector("double", ncol(df))  # 1. output
for(i in 1:ncol(df)) {                # 2. sequence; ncol(df) = start at this col, end at the last col
  output[i] <- median(df[[i]])        # 3. body
}
output
...
means <- c(0, 1, 2)
output <- double()                            # output: will be a double vector
for (i in seq_along(means)) {                 # sequence: "i along the sequence of means" "how long is means?" (3 positions), for loop will rin three times
  n <- sample(100, 1)                         # body: sample one random number from 1-100, creating a random sample size
  output <- c(output, rnorm(n, means[[i]]))   # body: rnorm generates random numbers from a normal distribution, n tells us how many, and means (center for teh distrib) come from the means vector (i^th position)
}
str(output)        ## inefficient

out <- vector("list", length(means)) # 3 empty lists waiting to be filled
...
Finally, the `while()` loop can be used with unknown sequence length.  This is used more in simulation than in data analysis.
 ...
 - Want to use for loops when we know how many times to do something. When we know when to start/stop

 - We want to use while loops to perform some task only while some condition is being met; only under ceratin conditions.

1. **output** (optional) - don't really need to create an empty vector/matrix
2.**define condition** to run the loop
3. **body** - what do we want or loop to actually do

**[Pause to Ponder:]** What does the following code do?

How many flips it takes to get three consecutive heads. 

```{r}
flip <- function() sample(c("T", "H"), 1)   # flip is either H or T once
flips <- 0                                  # setting flips equal to 0
nheads <- 0                                 # number of heads set to 0
while (nheads < 3) {                        # if number of heads is less than 3, keep running (once nheads reaches 3, or three heads are flipped in a row, the loop stops)
  if (flip() == "H") {                      # if flip is heads...
    nheads <- nheads + 1                    # number of heads increases by one
  } else {                                  # if flip is not heads...
    nheads <- 0                             # the number of heads goes back to zero
  }
  flips <- flips + 1                        # flips increases by 1
}
flips
```
...
gap1952 <- read_excel("~/SDS 264 Spring 2026/SDS_264_spring26/Class Activities/1952.xlsx")
gap1957 <- read_excel("~/SDS 264 Spring 2026/SDS_264_spring26/Class Activities/1957.xlsx")
```
