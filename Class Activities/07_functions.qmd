...
Three parts of a function
  ...
  1) descriptive, unique name
  
  2) arguments - the input for the function! 
  
    - example: ggplot is a function that needs inputs like a data set and aesthetics with variables that are required
  
  3) body - does the work!
  
```{r}
rescale0_1 <- function(x){    # arguments + name; x is place holder for the vector/data
  (x - min(x)) / (max(x) - min(x))  # rescales like we did above
}
...
temp0 <- c(4, 6, 8, 9, NA)
rescale01(temp0)      # NAs are contagious in summary functions
```
...
rescale01 <- function(x) {
  ...
  (x - rng[1]) / (rng[2] - rng[1]) # [1] first element, for minimum value ( range(temp)[1] ) and [2] for max value
}
rescale01(temp)
rescale01(c(0, 5, 10))
rescale01(c(-10, 0, 10))
...
```{r}
rescale_NAoption <- function(x, removeNAs = FALSE) {    # removeNAs = FALSE sets a default in the function
...
The problem is tidy evaluation, which makes most common coding easier, but makes some less common things harder.  Key terms to understand tidy evaluation:
  ...
  - env-variables = live in the environment (mpg) (can type it in the console on its own and have an output)
  - data-variables = live in data frame or tibble (cty) (will not give output when just typed into the console, R doesn't know where it lives)
...
greet <- function(time = x) {
  if(hour(time) <= 12) {
    return("good morning", call. = FALSE)
  }
}
