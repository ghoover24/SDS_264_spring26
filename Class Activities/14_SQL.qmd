...
`output.var = ""` is how you save the out put as an object in R
...
```{r}
airports %>%            # access directly from the server
  object.size() %>% 
  print(units = "Kb") 

airports %>%
  collect() %>%         # bring into R, more Kb
  object.size() %>%
  print(units = "Kb")
...
MSP_taxi <- flights |>
  filter(origin == "MSP",
         ...
         DayofMonth == 4) |>
  group_by(Reporting_Airline) |>
  summarize(avg_taxi_time = (TaxiOut + taxiin) / 2)
```
```{sql, connection = con}
DESCRIBE flightdata
```
...
SELECT
  ...
  AVG(taxiin) AS avg_taxi_it,
  AVG(taxiin + TaxiOut) AS avg_total_taxi
FROM flightdata
WHERE Year = 2023 AND Month = 5 AND DayofMonth = 4 AND origin = 'MSP'
GROUP BY Reporting_Airline
ORDER BY avg_total_taxi DESC;
```
