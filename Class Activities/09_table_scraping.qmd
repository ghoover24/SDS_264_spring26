...
- these main body and h3 are from control f in the html code, looking for American Cheese

```{r}
session <- bow("https://www.cheese.com/by_type", force = TRUE) # session is our url, ask if scraping is allowed
result <- scrape(session, query = list(t = "semi-soft", per_page = 100)) |> # adding secondary topics
  html_node("#main-body") |>   # identify main body
  html_nodes("h3") |>   # third header
  html_text()   # pulling just the text from the third header in the main body (just the names of the cheese)
head(result)
...
bind_cols(mpls_data1, mpls_data2) |>
  ...
  rename(avg_high = "Average high in",
         ...
         days_prec = "Days with precipitation",
         avg_prec = "Av. precipitation in",
...
pop3 <- as_tibble(pop2[,c(1:6,8)]) |>
  ...
  mutate(Estimate2024 = parse_number(Estimate2024),
         ...
         Change = str_extract(Change, ".\\d..[^ \\%]"),   # get rid of % but preserve +/- (optional),
